...
function isNighttime()
  ...
  local sunrise = hs.location.sunrise(currentLocation.latitude,
    currentLocation.longitude, dstOffset)
  local sunset = hs.location.sunset(currentLocation.latitude,
    currentLocation.longitude, dstOffset)
  ...
  if not sunrise or sunrise == "N/R" or not sunset or sunset == "N/R" then
    logError(
      "isNighttime: No sunrise or sunset found; using default time window")
...
function createDailyTask(resetTime, taskFunction, taskName, options)
  ...
  local function checkAndRunTask()
    ...
    local hasRunToday = (lastRunDay == currentDay)

    log("Checking if daily task should run", {
      hasRunToday = hasRunToday,
      lastRunDay = lastRunDay or "never",
    ...
    if not hasRunToday and (currentTime.hour > resetHour or
      (currentTime.hour == resetHour and currentTime.min >= resetMin)) then
      log("Attempting to run daily task: " .. taskName)
      local success, result = pcall(taskFunction)
      if not success then
        logError("Error running daily task " .. taskName .. ": " ..
                   tostring(result))
        return
      end
