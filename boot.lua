...
function isActualSystemBoot()
  ...
  log("Not an actual system boot, just a Hammerspoon restart/reload", {
    systemUptime = systemUptimeSeconds,
    lastBootTime = lastBootTime and os.date("%Y-%m-%d %H:%M:%S", lastBootTime) or
      "none"
  })
  return false
end
...
local function startColima()
  local output, status = hs.execute("colima start --ssh-agent --dns 8.8.8.8",
    true)
  if not status then
    ...
    if output:match("vz driver is running but host agent is not") then
      ...
      output, status =
        hs.execute("colima start --ssh-agent --dns 8.8.8.8", true)
      if not status then
...
local essentialApps = {"Messages", "Cursor", "Slack", "Notion Calendar",
                       "kitty", "Reminders", "Obsidian", "Vivid",
                       "Zen", "Notion", "Trello", "Hammerspoon",
                       "Wave Link"}
