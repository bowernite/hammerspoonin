...
local function setupPersistenceDir()
    ...
    if (not success or not err) and not hs.fs.attributes(persistence_dir) then
        ...
        if (not success or not err) and not hs.fs.attributes(persistence_dir) then
            logError("Failed to create fallback persistence directory: " ..
                         (type(err) == "string" and err or "unknown error"))
...
local function hasRestartedToday()
    ...
    local success, file = pcall(function()
        return io.open(restart_log_file, "r")
    end)
...
local function recordRestart()
    ...
    local success, file = pcall(function()
        return io.open(restart_log_file, "w")
    end)
    if not success or not file then
